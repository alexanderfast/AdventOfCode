...
def getHappiness(p, happy):
    for key in getPairs(p):
        if key in happy.keys():
            yield happy[key]
        else:
            yield 0
...
def part1(s):
    happy = {}
    names = set()
    for a, b, v in map(parse, (i for i in s.split("\n") if i != "")):
        happy[a, b] = v
        names.add(a)
    best = 0, None
    for p in itertools.permutations(names):
        h = list(getHappiness(p, happy))
        s = sum(h)
        #print p, h, s
        if s > best[0]:
            best = s, p
